//! GATE 18 @ L1 DISPATCH: does the alternating step probe find the free wins on
//! sao/mr/dickens while still holding samba/mozilla/x-ray at step 1?
...
fn main(){
    println!("L1: probe OFF (pinned step 1 on route 1) vs probe ON\n");
    println!("{:<14}{:>11}{:>10}{:>13}{:>10}{:>10}","corpus","size delta","size %","positions","pos %","verdict");
    ...
    for id in IDS{
        ...
        if a!=b || xa!=xb {
            let v=if b<=a && xb<xa {"FREE WIN"} else if b>a {"costs"} else {""};
            println!("{id:<14}{:>11}{:>9.3}%{:>13}{:>9.2}%{v:>10}",b-a,
                100.0*(b-a) as f64/a as f64, xa as i64-xb as i64,
                100.0*(xb as f64-xa as f64)/xa as f64);
        }
    }
    println!("\nTOTAL size {:+} bytes ({:+.4}%)   positions {:+} ({:+.2}%)",
