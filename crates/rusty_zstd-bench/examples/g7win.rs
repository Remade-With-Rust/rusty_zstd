//! GATE 7 @ L1: the tag is byte-identical, so this is purely speed.
//! PAIRED estimator (null-arm error ~0.1%); pooled-min is biased ~2.9%.
...
fn main() {
    ...
    for id in ids {
        ...
        let v = if mean < -1.0 && neg == 3 { w+=1; "TAG WINS" }
                else if mean > 1.0 && neg == 0 { l+=1; "tag loses" } else { "no signal" };
