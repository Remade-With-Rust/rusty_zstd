//! D10 adjudication: does the decode-ahead pipeline pay?
//!
...
fn once(z: &[u8], expect: &[u8], on: bool) -> f64 {
    rusty_zstd::set_pipeline_arm(on);
    rusty_zstd::prof_reset();
    let out = rusty_zstd::decompress(z).unwrap();
    assert!(out == expect, "pipeline={on}: OUTPUT CHANGED -- D10 is not byte-identical");
...
fn main() {
    ...
    let cap = 8usize << 20;
    println!("D10 DECODE-AHEAD PIPELINE @ L{lvl} -- DecSeqLoop stage ns/seq, ABBA, {rounds} rounds\n");
