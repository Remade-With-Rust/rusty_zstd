//! Same question, allocation removed. `decompress_into` reuses the destination,
//! so page faults and the allocator stop dominating a ~3% effect.
use std::time::Instant;
const IDS:&[&str]=&["zeros-32m","text-32m","incomp-32m","versions-16m","mozilla","webster","dickens","nci","xml","mr"];
const MB:&[usize]=&[2,8,32];
fn load(id:&str)->Option<Vec<u8>>{
    std::fs::read(format!("corpora/data/generated/{id}"))
        .or_else(|_|std::fs::read(format!("corpora/data/silesia/{id}"))).ok()
}
fn dt(z:&[u8],dst:&mut Vec<u8>,fused:bool,n:usize)->f64{
...
fn main(){
    println!("FUSED vs separate, ALLOCATION REMOVED (decompress_into, buffer reused)");
    println!("best-of-11 x ABBA x5. negative = fused FASTER\n");
    ...
    for id in IDS{
        ...
        for (i,m) in MB.iter().enumerate(){
            ...
            let _=rusty_zstd::decompress_into(&mut dst,&z).unwrap();   // warm the pages
            let (mut a,mut fu,mut nn)=(f64::MAX,f64::MAX,f64::MAX);
            for _ in 0..5{
                a=a.min(dt(&z,&mut dst,false,11));
                fu=fu.min(dt(&z,&mut dst,true,11));
                nn=nn.min(dt(&z,&mut dst,false,11));
            }
