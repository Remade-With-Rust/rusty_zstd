...
/// AVX2 MULTIVERSIONING (4.57), the enclosing-loop form.
///
/// 4.56 established that AVX2 cannot be delivered to a small hot site by
/// runtime dispatch: `#[target_feature]` forbids inlining into a baseline
/// caller, so a 2-instruction copy pays a ~7-instruction call and loses.
///
/// The fix is to dispatch at the LOOP instead. `decode_sequences_inner` is
/// `#[inline(always)]`, so it is inlined into BOTH an AVX2-enabled wrapper and a
/// baseline one. Inside the AVX2 copy every `copy_nonoverlapping(_, 32)` becomes
/// a single `vmovups` pair with no call at all -- the dispatch is one branch per
/// BLOCK rather than one call per copy.
///
/// Cost: the body is emitted twice.
#[allow(unsafe_code)]
pub(crate) fn decode_sequences(
    ...
    {
        if seqmv_on() && crate::simd::has_avx2() {
            // SAFETY: guarded by the runtime AVX2 check above.
            return unsafe {
                decode_sequences_avx2(
                    src, literals, out, window_size, block_max, state, dict,
                    frame_start, frame_skipped,
                )
            };
        }
    }
...
/// Arm for the multiversioned decoder. `false` forces the baseline body.
#[cfg(feature = "std")]
static SEQMV_ARM: core::sync::atomic::AtomicU8 = core::sync::atomic::AtomicU8::new(0);

/// Bench hook: `false` runs the baseline (non-AVX2) sequence loop.
#[cfg(feature = "std")]
pub fn set_seqmv_arm(on: bool) {
    SEQMV_ARM.store(u8::from(on) + 1, core::sync::atomic::Ordering::Relaxed);
}

#[cfg(feature = "std")]
#[inline(always)]
fn seqmv_on() -> bool {
    SEQMV_ARM.load(core::sync::atomic::Ordering::Relaxed) != 1
}
...
/// AVX2 instantiation of the sequence loop. The body is identical; only the
/// target features differ, so every fixed-width copy inside widens.
///
/// # Safety
