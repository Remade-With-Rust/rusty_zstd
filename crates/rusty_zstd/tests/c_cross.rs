...
// `bt_specialisation_covers_every_input_size` was removed in brick 8. It
// asserted every reachable `(hash_log, chain_log)` pair against
// `BT_SPEC_PAIRS`, but the dispatch that list fed had already been retired
// (`bt_resolve` returned the runtime body on every path), so the test was
// guarding a phantom: it passed while selecting nothing. The runtime body is
// exercised by every other bt-level test in this file.
